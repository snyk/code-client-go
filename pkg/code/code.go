...
func getSastSettingsConfig(engine workflow.Engine) configuration.DefaultValueFunction {
	...
	callback := func(c configuration.Configuration, existingValue any) (any, error) {
		...
		apiUrl := c.GetString(configuration.API_URL)
		org := c.GetString(configuration.ORGANIZATION)
		endpoint := apiUrl + "/v1/cli-config/settings/sast?org=" + url.QueryEscape(org)
		client := localNetworkStack.GetHttpClient()
		request, err := http.NewRequest(http.MethodGet, endpoint, nil)
		if err != nil {
			engine.GetLogger().Err(err).Msg("Failed to create request.")
			return nil, err
		}
		...
		return getSastResponse(response)
	}
	return callback
}

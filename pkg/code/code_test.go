...
func Test_Code_entrypoint_recordsSCLEInAnalytics(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			config.Set(configuration.ORGANIZATION, "aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee")
			networkAccess := networking.NewNetworkAccess(config)
			...
			assert.Equal(t, "native", (*body.Data.Attributes.Interaction.Extension)["implementation"])
		})
	}
}

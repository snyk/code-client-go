...
func AddDefaultHeaders(req *http.Request, requestId string, orgId string, method string, skipEncoding bool) {
	...
	if mustBeEncoded(method, skipEncoding) {
...
func EncodeIfNeeded(method string, requestBody []byte, skipEncoding bool) (*bytes.Buffer, error) {
	b := new(bytes.Buffer)
	if mustBeEncoded(method, skipEncoding) {
...
func mustBeEncoded(method string, skipEncoding bool) bool {
	return !skipEncoding && (method == http.MethodPost || method == http.MethodPut)
}
